...
def gather_logs(request, config, underlay):
    """Fixture executes Ansible command that gather logs from the K8s cluster nodes into a tarball and
    store results to the 'logs' directory. Logs collection starts if the test is failed and
    marked with 'gather_logs_if_test_failed' marker.
    ...
    def test_fin():
        if hasattr(request.node, 'rep_call') and request.node.rep_call.failed and collect_logs:
            remote = underlay.remote(host=config.k8s.kube_host)
            cmd = "/usr/bin/ansible-playbook " \
                  "--ssh-extra-args '-o\ StrictHostKeyChecking=no' -u {user} -b " \
                  "--become-user=root -i workspace/inventory/inventory.cfg " \
                  "-e searchpath=workspace " \
                  "-e @workspace/utils/kargo/roles/configure_logs/defaults/main.yml " \
                  "workspace/kargo/scripts/collect-info.yaml".format(user=settings.SSH_LOGIN)
            LOG.info(
            ...
            tarball_prefix = request.node.function.__name__ + '_' + datetime.now().strftime('%Y%m%d_%H%M%S') + '_'
            tarball_dst = settings.LOGS_DIR + '/logs/' + tarball_prefix + tarball_name
