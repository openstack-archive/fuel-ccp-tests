...
class YamlEditor(object):
    ...
    def __init__(self, file_path, ip=None, port=None,
                 username=None, password=None, keys=None,
                 document_id=0,
    ...
    def __get_file(self, mode="r"):
        if self.ip:
            ...
            return remote.open(self.file_path, mode=mode)
        else:
    ...
    def get_content(self):
        ...
        with self.__get_file() as file_obj:
            self.documents = [x for x in yaml.load_all(file_obj)]
            return self.documents[self.document_id]

    def write_content(self, content=None):
        if content:
            self.content = content
        self.documents(self.document_id) = self.content
        ...
        with self.__get_file("w") as file_obj:
            yaml.dump_all(self.documents, file_obj,
