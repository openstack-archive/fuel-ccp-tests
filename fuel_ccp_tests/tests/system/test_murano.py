...
class TestMurano(object):
    """docstring for TestPreCommitSahara"""

    @pytest.mark.murano_test
    @pytest.mark.murano_component
    @pytest.mark.revert_snapshot(settings.PRECOMMIT_SNAPSHOT_NAME)
    def test_deploy_os_with_custom_murano(
        ...
        Scenario:
            ...
            4. Fetch murano from review
            5. Fetch containers from external registry
            6. Build murano container
            7. Deploy Openstack
            8. Run tempest
        ...
        if settings.REGISTRY == '127.0.0.1:31500':
            ...
            ccpcluster.fetch()
            ccpcluster.update_service('murano')
