...
class TestAppController(base_test.SystemBaseTest):
    ...
    def k8s_obj_successful(self, k8sclient, obj):
        pods = k8sclient.pods
        serivces = k8sclient.services
        replicas = k8sclient.replicasets
        jobs = k8sclient.jobs
        successful = {
            "pod": (
                lambda x: pods.get(name=x).status.phase == "Running"
            ),
            "job": (
    ...
    @pytest.mark.ac_linear_test_k8sclient
    @pytest.mark.snapshot_needed
    @pytest.mark.revert_snapshot(ext.SNAPSHOT.k8s_deployed)
    @pytest.mark.fail_snapshot
    def test_linear_by_k8sclient(self, k8scluster):
        """Doc string"""
    ...
    def test_linear_manually(self, underlay, k8scluster):
        ...
        remote = underlay.remote(node_name=underlay.node_names()[0])
        podphase = (
            lambda x: k8sclient.pods.get(name=x).status.phase
        )
