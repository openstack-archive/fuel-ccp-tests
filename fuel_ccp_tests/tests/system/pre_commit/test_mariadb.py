...
class TestPreCommitMariadb(object):
    ...
    def test_deploy_os_with_custom_mariadb(
        ...
        cmd = "docker ps | grep mariadb | grep -v POD"
        res = remote.check_call(cmd)

        docker_id = res['stdout'][0].split()[0]
        LOG.info("Docker ID is {0}".format(docker_id))

        cmd = "mysql  -uroot -ppassword -e 'SHOW DATABASES;'"

        docker_exec = 'docker exec -i {docker_id} bash -c "{cmd}"'.format(
            docker_id=docker_id, cmd=cmd)
        result = remote.check_call(docker_exec)
