...
class TestNova(object):

    @pytest.mark.revert_snapshot(ext.SNAPSHOT.initial)
    @pytest.mark.pre_commit_nova
    @pytest.mark.fail_snapshot
    def test_build_deploy_nova(self, config, underlay,
                               k8scluster, ccpcluster):
        """Test build, deploy and verify nova service
        Scenario:
        1. Fetch all repositories and update nova
        ...
        # test nova compute with tempest
        command = [
            'chmod +x {0}'.format(script_path),
            'cd {0}; ./{1} "--regex tempest.api.compute"'.format(
