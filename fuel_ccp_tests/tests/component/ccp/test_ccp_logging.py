...
@pytest.yield_fixture(scope='function')
def elastic_client_public(os_deployed_stacklight, k8s_actions, config):
    cluster = k8s_actions.api
    service_list = cluster.services.list(namespace='ccp')
...
@pytest.yield_fixture(scope='function')
def elastic_client_private(os_deployed_stacklight,
                           k8s_actions,
                           admin_node,
                           config):
    cluster = k8s_actions.api
...
class TestCppLogging(object):
    ...
    def test_logging_kibana_running_single_node(
        ...
        show_step(1)
        cluster = k8s_actions.api
        pods_list = cluster.pods.list(namespace='ccp')
        kibana_count = len(
    ...
    def test_logging_heka_running_all_nodes(
        ...
        cluster = k8s_actions.api
        pods_list = cluster.pods.list(namespace='ccp')
        nodes_list = [node.name for node in cluster.nodes.list()]
        heka_nodes = [
    ...
    def test_logging_log_rotate_for_mysql(
        ...
        2. Repeat steps 7,8(create and trigger) for 9 times
        3. Create mysql.log file required for log rotate in cron pod
        4. Trigger dayly cron logrotate task
        5. Ensure that count of log files is equal to 7
        """
        ...
        show_step(1)
        # get cron pod
        cron_pod = [pod for pod in k8sclient.pods.list(
            namespace=ext.Namespace.BASE_NAMESPACE)
