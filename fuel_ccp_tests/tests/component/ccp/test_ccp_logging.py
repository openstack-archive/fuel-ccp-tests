...
@pytest.yield_fixture(scope='function')
def elastic_client_public(deploy_openstack_stacklight, admin_node, config):
...
@pytest.yield_fixture(scope='function')
def elastic_client_private(deploy_openstack_stacklight, admin_node, config):
...
@pytest.mark.revert_snapshot(ext.SNAPSHOT.os_stacklight_deployed)
class TestCppLogging(object):
    ...
    def test_logging_search_for_logs_from_all_running_heka_instances(
        ...
        hekas = filter(lambda x: 'heka' in x.name,
            k8sclient.pods.list(namespace=ext.Namespace.BASE_NAMESPACE))
    ...
    def test_logging_trigger_event_into_rabitmq(self,
        ...
        k8sclient = k8scluster(default_namespace=ext.Namespace.BASE_NAMESPACE)
    ...
    def test_logging_log_rotate_for_mysql(
        ...
        k8sclient = k8scluster(default_namespace=ext.Namespace.BASE_NAMESPACE)
        # get cron pod
        cron_pod = filter(lambda x: 'cron-' in x.name,
            k8sclient.pods.list(namespace=ext.Namespace.BASE_NAMESPACE))[0]
        # clean files
