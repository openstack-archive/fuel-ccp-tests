...
def make_file(admin_node):
    ...
    admin_node.check_call('dd if=/dev/zero of=/tmp/{0} '
                          'bs=1M count={1}'.format(filename, size / 1024**2))
    yield size
    admin_node.check_call('rm /tmp/{}'.format(filename))
...
class TestMetrics(object):
    def get_influxdb_client(self,
                            ...
                            password='root',
                            db='ccp'):
        ep = k8sclient.endpoints.get(name='influxdb', namespace='ccp')
        return InfluxDBClient(ep.subsets[0].addresses[0].ip,
                              ep.subsets[0].ports[0].port, login, password, db)
    ...
    def test_filesystem_metrics(self, request, k8s_actions, admin_node):
        ...
        dbclient = self.get_influxdb_client(k8sclient)
        for serie in series:
        ...
        # Create file
        request.getfixturevalue('make_file')
