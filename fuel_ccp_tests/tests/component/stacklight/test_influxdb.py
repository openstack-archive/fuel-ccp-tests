...
@pytest.fixture
def make_influxdb_query(k8s_actions, admin_node):
...
class TestMetrics(object):
    ...
    def test_filesystem_metrics(self, make_file, make_influxdb_query,
                                admin_node):
        ...
        query = ("SHOW MEASUREMENTS WITH MEASUREMENT "
                 "=~ /intel\.procfs\.filesystem*/")
        results = make_influxdb_query(query)
        series = [x['name'] for y in results for x in y['measurements']]

        for serie in series:
            query = ('select count(value) from "{}" '
                     'where time > now() - 1d').format(serie)
            results = make_influxdb_query(query)
            count = results[0][serie][0]['count']
            err_msg = "There is no records for serie `{}` in influxdb".format(
                serie)
        ...
        data = self.get_influxdb_last_record(make_influxdb_query,
                                             free_space_serie,
