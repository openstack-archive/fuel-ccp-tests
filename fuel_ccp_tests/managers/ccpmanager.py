...
class CCPManager(object):
    ...
    def run(self, cmd, components=None, params=None, suppress_output=False):
        ...
        if components:
            ...
            components = ''
        if suppress_output:
            #ccp_out_redirect = "> /dev/null 2>&1"
            ccp_out_redirect = ("> >(tee ccp.out.log > /dev/null) "
                                "2> >(tee ccp.err.log >/dev/null)")
        else:
            #ccp_out_redirect = "2> /dev/null"
            #ccp_out_redirect = "> >(tee ccp.log) 2> >(tee ccp.err.log >&2)"
            ccp_out_redirect = ""
    ...
    def update_service(self, service_name, path=None):
        ...
        with self.__underlay.remote(
            ...
            remote.upload(
                path,
                "/home/vagrant/microservices-repos/fuel-ccp-{}".format(
                    service_name))
