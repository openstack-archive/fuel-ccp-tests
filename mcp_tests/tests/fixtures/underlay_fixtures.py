...
from mcp_tests.models import envmanager
from mcp_tests.models import envmanager_empty
...
def hardware(config):
    ...
    if manager is None:
        ...
        env = envmanager.EnvironmentManager(config=config)
    else:
        raise Exeption("Unknown environment manager set: '{}'".format(manager))
...
def revert_snapshot(request, hardware):
    ...
    snapshot_name = extract_name_from_mark(revert_snapshot, None)
