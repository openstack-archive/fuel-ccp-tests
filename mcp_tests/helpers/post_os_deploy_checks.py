...
def check_pods_status(k8sclient, timeout=600):
    def is_pod_running(cluster, pod_name):
        return lambda: (cluster.pods.get(name=pod_name)
...
# TODO: replace check with deployment status request
def check_jobs_status(k8sclient, timeout=600):
    def is_job_running(cluster, job_name):
        return lambda: (cluster.jobs.get(name=job_name)
