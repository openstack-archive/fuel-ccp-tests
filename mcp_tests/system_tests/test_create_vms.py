...
class TestCreateEnv(object):
    ...
    def test_deploy_microservices(self):
        """
        CCPINSTALLER should contain a local path to the cloned repository:
        "git clone https://git.openstack.org/openstack/fuel-ccp-installer"
        MICROSERVICES should contain a local path to the cloned repository:
        "git clone https://git.openstack.org/openstack/fuel-ccp"
        REGISTRY should contain URL on registry in the case BUILD_IMAGES=False
