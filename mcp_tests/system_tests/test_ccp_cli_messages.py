...
class CliMessages(object):
    ...
    options_list = ["[--repositories-clone]",
                    "[--repositories-clone-concurrency REPOSITORIES_CLONE_CONCURRENCY]",
                    "[--repositories-fuel-ccp-debian-base REPOSITORIES_FUEL_CCP_DEBIAN_BASE]",
                    "[--repositories-fuel-ccp-entrypoint REPOSITORIES_FUEL_CCP_ENTRYPOINT]",
                    "[--repositories-fuel-ccp-etcd REPOSITORIES_FUEL_CCP_ETCD]",
                    "[--repositories-fuel-ccp-glance REPOSITORIES_FUEL_CCP_GLANCE]",
                    "[--repositories-fuel-ccp-heat REPOSITORIES_FUEL_CCP_HEAT]",
                    "[--repositories-fuel-ccp-horizon REPOSITORIES_FUEL_CCP_HORIZON]",
                    "[--repositories-fuel-ccp-keystone REPOSITORIES_FUEL_CCP_KEYSTONE]",
                    "[--repositories-fuel-ccp-mariadb REPOSITORIES_FUEL_CCP_MARIADB]",
                    "[--repositories-fuel-ccp-memcached REPOSITORIES_FUEL_CCP_MEMCACHED]",
                    "[--repositories-fuel-ccp-neutron REPOSITORIES_FUEL_CCP_NEUTRON]",
                    "[--repositories-fuel-ccp-nova REPOSITORIES_FUEL_CCP_NOVA]",
                    "[--repositories-fuel-ccp-openstack-base REPOSITORIES_FUEL_CCP_OPENSTACK_BASE]",
                    "[--repositories-fuel-ccp-rabbitmq REPOSITORIES_FUEL_CCP_RABBITMQ]",
                    "[--repositories-fuel-ccp-stacklight REPOSITORIES_FUEL_CCP_STACKLIGHT]",
    ...
    usage_show_dep_message = "usage: ccp show-dep [-h] components [components ...]\n\n"
    ...
    error_message_unrecognized_arguments = "ccp: error: unrecognized arguments: {}"
...
def assert_substring_exist(self, log_message_text, source_text, expected_text, expected_count=1):
...
class TestCppCliNormalMessageInShowDep(object):
    ...
    def test_component_have_single_dependency(self, admin_node):
        ...
        assert len(
            StringHelper.reduce_occurrences(dependencies, StringHelper.extract_string_from_iter(
                filter(lambda s: "fuel_ccp.fetch" not in s, result['stdout']))).strip()) == 0
    ...
    def test_component_have_multiple_dependencies(self, admin_node):
        ...
        assert len(
            StringHelper.reduce_occurrences(dependencies, StringHelper.extract_string_from_iter(
                filter(lambda s: "fuel_ccp.fetch" not in s,
    ...
    def test_component_have_no_dependencies(self, admin_node):
        ...
        cmd = "ccp show-dep {}".format(component[0])
        result = admin_node.check_call(cmd, expected=[0])
        assert StringHelper.reduce_occurrences([no_dependencies_message],
                                               StringHelper.extract_string_from_iter(result['stdout']))
    ...
    def test_component_help_message_via_short(self, admin_node):
        ...
        cmd = "ccp show-dep -h"
        result = admin_node.check_call(cmd, expected=[0])
        assert len(StringHelper.reduce_occurrences([CliMessages.usage_show_dep_message + CliMessages.help_message],
                                                   StringHelper.extract_string_from_iter(
                                                       result['stdout'])).strip()) == 0
    ...
    def test_multiple_components_given(self, admin_node):
        logger.info("Show info for several components with dependencies no cross-referenced")
        component = ["keystone", "memcached"]
        dependencies = ["mariadb", "etcd"]
        cmd = "ccp show-dep {}".format(" ".join(component))
        result = admin_node.check_call(cmd, expected=[0])
        assert len(
            StringHelper.reduce_occurrences(dependencies, StringHelper.extract_string_from_iter(result['stdout']))) == 0

    @pytest.mark.fail_snapshot
    def test_multiple_components_given_cross_reference(self, admin_node):
        logger.info("Show info for component with multiple dependencies cross referenced dependecies  ")
...
class TestCppCliErrorMessageInShowDep(object):
    ...
    def test_nonexistent_component_given(self, admin_node):
        ...
        expected = 1
        error_message = CliMessages.error_message_bad_component_name.format(component[0])
        cmd = "ccp show-dep {}".format(component[0])
        result = admin_node.check_call(cmd, expected=[expected])
        assert error_message in StringHelper.extract_string_from_iter(result['stderr'])
    ...
    def test_no_components_given(self, admin_node):
        ...
        result = admin_node.check_call(cmd, expected=[expected])
        assert len(
            StringHelper.reduce_occurrences([CliMessages.error_message, CliMessages.usage_show_dep_message],
                                            StringHelper.extract_string_from_iter(result['stderr'])).strip()) == 0
        assert len(StringHelper.extract_string_from_iter(result['stdout'])) == 0
    ...
    def test_unexpected_option_given(self, admin_node):
        ...
        assert len(StringHelper.reduce_occurrences(
            [CliMessages.usage_message,
             CliMessages.error_message_unrecognized_arguments.format(argrument)] + CliMessages.options_list,
            StringHelper.extract_string_from_iter(
