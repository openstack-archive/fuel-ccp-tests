...
from mcp_tests.system_tests.pre_commit.base_test import ServiceBase

LOG = logger.logger
LOG.addHandler(logger.console)


class TestServiceHorizon(ServiceBase):

    dockerfile = '''
    FROM {registry}/{tag}/horizon
    RUN apt-get install -y xvfb
    RUN apt-get install -y firefox-esr
    COPY local-horizon.conf /horizon-master/openstack_dashboard/test/integration_tests
    ...
    def test_horizon_component(self, env, k8sclient):
        ...
        result = remote.execute(
            ...
            " bash -c 'cd horizon-master; ./run_tests.sh -V'".format(settings.SSH_LOGIN))
        assert result['exit_code'] == 0, "Horizon unit tests failed"
