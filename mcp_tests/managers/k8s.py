    ...
    def install_k8s(cls, env, custom_yaml=None, env_var=None):
        ...
        if not env.has_snapshot(snapshot_name):
            ...
            if custom_yaml is None:
                LOG.info("Use test environment defined custom yaml")
                LOG.debug("Provided custom_yaml = {}".format(custom_yaml))
                environment_variables['CUSTOM_YAML'] = yaml.dump(
                    settings.DEFAULT_CUSTOM_YAML, default_flow_style=False)
            elif custom_yaml is not False:
                LOG.info("Use provided custom yaml")
                LOG.debug("Provided custom_yaml = {}".format(custom_yaml))
                environment_variables['CUSTOM_YAML'] = yaml.dump(
                    custom_yaml, default_flow_style=False)
            else:
                LOG.info("Use kargo default custom yaml. "
                         "CUSTOM_YAML isn't setted")
