...
def hardware(config):
    ...
    if manager == 'devops':
        ...
        config_ssh = []
        snapshot_name = UNDERLAY_SNAPSHOT_SUFFIX
        env = envmanager.EnvironmentManager(config_file=config.hardware.conf_path)
        ...
        if config.underlay.ssh == []:
            ...
            for d_node in env.k8s_nodes:
                ssh_data = {
                    ...
                    'address_pool': env.get_network_pool(ext.NETWORK_TYPE.public).address_pool.name,
...
def underlay(config, hardware):
    """Fixture that should provide SSH access to underlay objects.
       ...
       Input data:
        - config.underlay.ssh : JSONList, *must* be provided, from 'hardware' fixture
                                or from an external config

    :rtype: Object that can select and return nodes.
    """
    return UnderlaySSHManager(config.underlay.ssh)
